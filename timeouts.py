...
df = df.merge(team_lookup, on=['INITIALS'], how='left')
df = df.merge(schedule[['ROAD TEAM', 'HOME TEAM', 'game_id']], on=['game_id'], how='left')
df['team_id'] = df['SHORT NAME']
df['point_diff'] = df.apply(lambda row: row['away_score'] - row['home_score'] if row['ROAD TEAM'] == row['INITIALS'] else row['home_score'] - row['away_score'], axis=1)
