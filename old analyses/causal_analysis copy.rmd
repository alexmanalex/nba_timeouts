...
```{r}
lin_fit = glm(formula = treatment~scoring_run + coach_exp + point_diff + time_since_last_break,
   data=df, 
   family=binomial())
...
```{r}
library(mgcv)
np_fit = gam(treatment~s(scoring_run) + s(coach_exp) + s(point_diff) + s(time_since_last_break),
...
```{r}
glinfit = lm(outcome ~ treatment + scoring_run + coach_exp + point_diff + time_since_last_break, data=match.data(m))
```
...
fc <- function(df_positive,i){
  ...
  outcome_model = lm(outcome~treatment+scoring_run+coach_exp+point_diff+time_since_last_break, data=df_positive_boot)
...
fc <- function(df_negative,i){
  ...
  outcome_model = lm(outcome~treatment+scoring_run+coach_exp+point_diff+time_since_last_break, data=df_negative_boot)
